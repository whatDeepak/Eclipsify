...
class BreathingAdapter(private val item: List<Breathing>,
                       private val activityClasses: List<Class<out AppCompatActivity>>,
):
    RecyclerView.Adapter<BreathingAdapter.BreathingHolder>(){

    // ViewHolder class
    class BreathingHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val title: TextView = itemView.findViewById(R.id.breatheTitle)
        val cardView: CardView = itemView.findViewById(R.id.breatheCardView) // Replace with the actual ID of your CardView
        val time: TextView = itemView.findViewById(R.id.breatheTime)
        val image: ImageView = itemView.findViewById(R.id.breatheImageView)
    }

    // Create new ViewHolders (invoked by the layout manager)
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BreathingHolder {
        val itemView = LayoutInflater.from(parent.context)
            .inflate(R.layout.breathing_item, parent, false)
        return BreathingHolder(itemView)
    }


    // Replace the contents of a ViewHolder (invoked by the layout manager)
    override fun onBindViewHolder(holder: BreathingHolder, position: Int) {
